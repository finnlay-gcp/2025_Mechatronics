...
def gui_send_command():
    # Helper to call the send function from GUI button
    # We use the 'current_' global variables tracked in the loop
    send_packet_now(current_angle, current_dist, current_code)
    # Update sent vars manually here for immediate GUI feedback
...
ttk.Label(frm_ctrl, text="* Ensure Robot ID is visible to calculate values", font=("Arial", 8, "italic")).pack(pady=5)
...
def send_packet_now(angle_deg: float, dist_mm: float, mode_code: float):
    ...
    target_ip = var_pi_ip.get() # Get from GUI
    target_port = var_pi_port.get() # Get from GUI
    try:
...
# ============================================================
def open_camera():
    for idx in [0]: # Added 0 as fallback
...
print("=== START ===")
print("PRESS SPACEBAR (OR GUI BUTTON) TO SEND UDP COMMANDS")
print(f"Initial goal: {goal_id}")

try:
    while True:
        # 1. Update GUI backend (process events)
        try:
        ...
        if state == STATE_WAITING:
            ...
            if WAIT_REQUIRES_UDP_SIGNAL and not wait_udp_ok:
                ...
                if waiting_at_id == QUARANTINE_ID:
                    # If at Quarantine: Wait only, NO CHARGING
                    battery_level = 0.0
                    draw_status(frame, f"AT {waiting_at_id}: QUARANTINE HOLD {remaining:0.1f}s", y=60)
                    # Note: draw_battery_ui is NOT called here
                else:
                    # If at Door/Other: Simulate Charging
        ...
        # Determine Robot Visibility for UI
        robot_vis = (ROBOT_ID in pose_by_id)
        var_robot_visible.set(robot_vis)
        if robot_vis:
