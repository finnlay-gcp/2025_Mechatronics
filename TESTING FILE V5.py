...
# Only consider these IDs (ignores everything else, reduces confusion)
...
# --- CHANGED: Added force_now parameter ---
def udp_send(angle_deg: float, dist_mm: float, mode_code: float, force_now: bool = False):
    global last_udp_send_time
    now = time.time()
    
    # Send if forced (safety stop) OR if the timer has elapsed
    if force_now or (now - last_udp_send_time) >= UDP_INTERVAL_SEC:
...
# --- NEW FUNCTION: Displays the current UDP values on screen ---
def draw_udp_values(frame, angle, dist, code):
    text = f"UDP SEND -> Ang: {angle:.0f} | Dist: {dist:.0f} | Code: {code:.0f}"
    # Drawing in Pink/Magenta to differentiate from other status text
    cv2.putText(frame, text, (40, 160), cv2.FONT_HERSHEY_SIMPLEX, 0.9, (255, 0, 255), 3)

def draw_battery_ui(frame, progress_0_to_1: float, title="SCREAM ENERGY"):
    progress = float(np.clip(progress_0_to_1, 0.0, 1.0))
    x, y = 40, 200 # Moved down slightly to make room for UDP text
    bw, bh = 420, 80
    ...
    for idx in [0, 1, 2]:
        ...
        if cap_try.isOpened():
            ...
            if ret and frame is not None:
                ...
                return cap_try
            cap_try.release()
    raise RuntimeError("Could not open camera. Close Zoom/Teams/Camera app and try again.")
...
try:
    ...
    print("Loaded camera calibration from workdir/Calibration.npz")
except Exception:
    print("No calibration file found; using approximate camera intrinsics.")
...
def decide_mode_code(angle_deg, dist_mm, escape_mode=False):
    ...
    if escape_mode and USE_ESCAPE_MODE_CODE:
        return angle_rounded, dist_rounded, ESCAPE_MODE_CODE

    if abs(angle_rounded) > ANGLE_TOLERANCE_DEG:
        code = 1.0
    else:
        code = 2.0

    if dist_rounded <= DIST_STOP_MM:
        code = 3.0

    return angle_rounded, dist_rounded, code

def compute_angle_and_distance(pose_by_id, target_id):
    if ROBOT_ID not in pose_by_id or target_id not in pose_by_id:
        return None
...
print("=== START ===")
print(f"ROBOT_ID={ROBOT_ID} HOME_ID={HOME_ID} DOORS={DOOR_IDS} SPECIAL_DOOR={SPECIAL_DOOR_ID} QUARANTINE_ID={QUARANTINE_ID}")
...
while True:
    ...
    # Build approximate intrinsics once we know frame size
    if CM is None:
        h, w = frame.shape[:2]
        fx = fy = 0.9 * w
        cx, cy = w / 2.0, h / 2.0
    ...
    corners_list, ids, rejected = detect_markers(gray)
    ...
    if state == STATE_WAITING:
        udp_send(0.0, 0.0, 3.0)
        draw_udp_values(frame, 0.0, 0.0, 3.0) # Show UDP status
    ...
    # ---------------- GO TO GOAL ----------------
        ...
        if res is None:
            ...
            # --- CHANGED: Force stop immediately if tracking lost ---
            udp_send(0.0, 0.0, 3.0, force_now=True)
            draw_udp_values(frame, 0.0, 0.0, 3.0) # Show UDP status
        else:
            angle_deg, dist_mm, center_r, center_t = res
            cv2.line(frame, center_r, center_t, (255, 0, 0), 3)

            cv2.putText(frame, f"Dist {dist_mm:.0f}mm  Ang {angle_deg:.0f}",
                        (40, 110), cv2.FONT_HERSHEY_SIMPLEX, 0.9, (0, 255, 255), 2)

            if dist_mm < TARGET_REACH_THRESHOLD_MM:
                # --- CHANGED: Force stop immediately when target reached ---
                udp_send(0.0, 0.0, 3.0, force_now=True)
                draw_udp_values(frame, 0.0, 0.0, 3.0) # Show UDP status

                if goal_id in DOOR_IDS:
                    state = STATE_WAITING
                    waiting_at_id = goal_id
                    wait_started = False
                    wait_udp_ok = (not WAIT_REQUIRES_UDP_SIGNAL)

                elif goal_id == QUARANTINE_ID:
                    state = STATE_WAITING
                    waiting_at_id = goal_id
                    wait_started = False
                    wait_udp_ok = (not WAIT_REQUIRES_UDP_SIGNAL)
                ...
                # Normal navigation uses the 10-second pulse
                udp_send(ang_r, dist_r, code)
                draw_udp_values(frame, ang_r, dist_r, code) # Show UDP status
