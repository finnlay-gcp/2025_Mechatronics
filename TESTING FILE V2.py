...
try:
    ...
    sock_receive.setblocking(False) # <--- CRITICAL: Allows code to run while listening
...
# Added a small check to prevent crash if file is missing
try:
...
signal_received = False    # NEW: Have we received the '1' from UDP yet?
...
while True:
    ...
    if wait_mode:
        ...
        # --- NEW LOGIC: CHECK FOR UDP SIGNAL '1' ---
        if not signal_received:
            try:
                # Try to get data without stopping the program
                data, addr = sock_receive.recvfrom(BUFFER_SIZE)
                if len(data) >= 8:
                    val = struct.unpack('<d', data[:8])[0]
                    if val == 1.0:
                        print(f"Signal '1' received! Starting {WAIT_DURATION}s timer.")
                        signal_received = True
                        wait_start_time = time.time() # Start the 10s timer NOW
            except socket.error:
                pass # No data, keep waiting
                
            # Keep resetting start time so timer doesn't tick down yet
            if not signal_received:
                wait_start_time = time.time()
                msg = f"TARGET {frozen_target_id} REACHED. WAITING FOR UDP '1'..."
        else:
            # Signal received, now we count down
            elapsed = time.time() - wait_start_time
            remaining = WAIT_DURATION - elapsed
            msg = f"MOVING TO NEXT IN: {remaining:.1f}s"
        ...
        # Check if wait is over (Only happens if signal_received is True)
        if signal_received and (time.time() - wait_start_time >= WAIT_DURATION):
            ...
            signal_received = False # Reset for next time
            ...
            if 0 in flat_ids:
                ...
                if len(candidates) > 0:
                    ...
                    if min_dist < TARGET_REACH_THRESHOLD:
                        ...
                        print(f"!!! Entering WAIT MODE. Waiting for UDP '1'... !!!")
                        
                        # Trigger Wait Mode
                        wait_mode = True
                        signal_received = False # Reset signal flag
                        wait_start_time = time.time()
    ...
    # [REMOVED BLOCKING FUNCTION FROM HERE]
