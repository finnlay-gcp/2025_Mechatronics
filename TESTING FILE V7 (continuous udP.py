...
# --- FIXED: Lowered for responsiveness (was 1.0) ---
UDP_INTERVAL_SEC = 2
...
# --- NEW: Track the last code sent to detect changes ---
last_sent_code = -1.0 

def udp_send(angle_deg: float, dist_mm: float, mode_code: float, force_now: bool = False):
    global last_udp_send_time, last_sent_code
    now = time.time()
    
    # --- LOGIC FIX: Force send if the Mode Code has changed (e.g. STOP -> GO) ---
    if mode_code != last_sent_code:
        force_now = True

    # Send if forced OR if timer elapsed
    if force_now or (now - last_udp_send_time) >= UDP_INTERVAL_SEC:
        ...
        last_sent_code = mode_code # Update memory
...
def draw_udp_values(frame, angle, dist, code):
    # Pink text showing exactly what is being sent
...
while True:
    ...
    if state == STATE_WAITING:
        ...
        draw_udp_values(frame, 0.0, 0.0, 3.0)
        ...
        if res is None:
            ...
            if dist_mm < TARGET_REACH_THRESHOLD_MM:
                ...
                # The udp_send logic handles forcing the packet if code changes (e.g. 3.0 -> 2.0)
                udp_send(ang_r, dist_r, code)
                draw_udp_values(frame, ang_r, dist_r, code)
